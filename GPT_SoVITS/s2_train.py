import os, warnings, traceback, logging
from random import randint

# Suppress extraneous logs and warnings
...
# Use the new AMP API from torch.amp
...
torch.set_float32_matmul_precision("medium")  # lowest precision but fastest

global_step = 0
device = "cpu"  # fallback device (e.g. for mps in the future)
# Define the device type for AMP
...
def run(rank, n_gpus, hps):
    ...
    # Uncomment and set up eval_dataset/eval_loader if needed.
    # eval_dataset = TextAudioSpeakerLoader(hps.data.validation_files, hps.data, val=True)
    # eval_loader = DataLoader(eval_dataset, num_workers=0, shuffle=False, batch_size=1, pin_memory=True, drop_last=False, collate_fn=collate_fn)
    ...
    net_d = (MultiPeriodDiscriminator(hps.model.use_spectral_norm).cuda(rank)
             if torch.cuda.is_available() else MultiPeriodDiscriminator(hps.model.use_spectral_norm).to(device))
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, (ssl, ssl_lengths, spec, spec_lengths, y, y_lengths, text, text_lengths) in tqdm(enumerate(train_loader)):
        ...
        # Generator and Discriminator forward passes using new autocast API
        with autocast(device_type=amp_device, enabled=hps.train.fp16_run):
            (y_hat, kl_ssl, ids_slice, x_mask, z_mask,
             (z, z_p, m_p, logs_p, m_q, logs_q),
             stats_ssl) = net_g(ssl, spec, spec_lengths, text, text_lengths)
    ...
    if epoch % hps.train.save_every_epoch == 0 and rank == 0:
        ...
        if rank == 0 and hps.train.if_save_every_weights:
            ...
            logger.info("saving ckpt %s_e%s:%s" %
                        (hps.name, epoch,
                         savee(ckpt, hps.name + "_e%s_s%s" % (epoch, global_step), epoch, global_step, hps)))
    if rank == 0:
